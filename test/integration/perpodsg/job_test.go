	...
	Describe("Jobs", func() {
		...
		JustBeforeEach(func() {
			...
			serverContainer := manifest.NewBusyBoxContainerBuilder().
				Name("sgp-server").
				Image("577579559164.dkr.ecr.eu-isoe-west-1.cloud.adc-e.uk/networking-e2e-test-images/nginx:1.25.2_amd64").
			...
			for i := 0; i < testNodeCount; i++ {
				...
				for j := 0; j < numJobs; j++ {
					...
					jobContainer := manifest.NewBusyBoxContainerBuilder().
						Image("577579559164.dkr.ecr.eu-isoe-west-1.cloud.adc-e.uk/networking-e2e-test-images/curlimages/curl:latest").
